...
export default class BookModel {
  ...
  public async patch(id: number, data: Partial<Book>): Promise<Book> {
    const propsNContent = Object.entries(data);

    const arrayOfPromises = propsNContent.map(async (each) => {
      const [prop, content] = each;
      
      await this.connection.execute(
        `UPDATE books SET ${prop}=? WHERE id=?`,
        [content, id],
      );
    });
